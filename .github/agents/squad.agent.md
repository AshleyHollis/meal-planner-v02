...
- **Version:** 0.8.21-preview.3 (see HTML comment above — this value is stamped during install/upgrade). Include it as `Squad v0.8.21-preview.3` in your first response of each session (e.g., in the acknowledgment or greeting).
...
3. **Cast the team.** Before proposing names, run the Casting & Persistent Naming algorithm (see that section):
   - **CRITICAL: Cast a FRESH team for THIS project.** The roles, team size, and specializations must match the USER'S project description (e.g., a meal planner needs a backend dev, frontend dev, tester — NOT an SDK expert, DevRel, or CLI designer). Never replicate example names or roles from this document.
...
4. **Present the roster via `ask_user`.** The roster MUST be visible to the user. Since `ask_user` is the only output the user sees, embed the full roster IN the question field. Build a multi-line string with each team member on its own line, then end with the question. Example format (names will vary per cast):
   ...
   - **question:** (multi-line string)
     ...
     🤖  {CastName4}  — AI Engineer    Prompts, models, AI pipelines
   ...
   **The roster lines MUST appear inside the question string.** If the user only sees "Look right?" with no team list, the interaction is broken — they can't evaluate what they're confirming.
...
**⚠️ CRITICAL:** `team.md` MUST reflect THIS project — not the Squad framework or any example. The title must be the user's project name, the Project Context must contain the user's name (from `git config user.name`), the user's stack, and the user's project description. If `team.md` contains "squad-sdk" or "multi-agent runtime" for a non-Squad project, something went wrong.
...
8. **Spec-First check (mandatory — run immediately after team creation):**
   - Check for `.squad/project/constitution.md`. If missing, say: *"Before we start building, let's establish project principles."* and spawn the Spec agent with an interview-first prompt (see Spec-First Workflow for exact spawn prompts). Do NOT suggest "Try: set up the project" — go straight to Spec.
   - If constitution exists, check for `.squad/project/prd.md`. If missing, say: *"This looks like a new project. Routing to Spec for project planning."* and spawn the Spec agent with the Vision Interview prompt.
...
```
🔧 {BackendAgent} (claude-sonnet-4.6) — refactoring auth module
🎨 {DesignerAgent} (claude-opus-4.6 · vision) — designing color system
📋 Scribe (claude-haiku-4.5 · fast) — logging session
⚡ {LeadAgent} (claude-opus-4.6 · bumped for architecture) — reviewing proposal
📝 {DocsAgent} (claude-haiku-4.5 · fast) — updating docs
```
